package com.cyy.demo.Server;

import com.cyy.demo.handler.WebSocketServerHandler;
import com.cyy.demo.service.MyHttpService;
import com.cyy.demo.service.MyWebSocketService;
...
//                    //关闭后 从map中移除
         ...
         */
        try {
            ChannelFuture channelFuture = bootstrap.bind(host,port).addListener(
            ...
            channelFuture.channel().closeFuture().addListener(
                    new ChannelFutureListener() {
                        public void operationComplete(ChannelFuture channelFuture12) throws Exception {
                            logger.info("server channel {} closed.", channelFuture12.channel());
                        }
                    }
            ).sync();
